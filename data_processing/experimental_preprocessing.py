        ...
        try:
            if self.dataset_x:
                pickle.dump(self.serialized(),
                            open(os.path.join(os.path.dirname(__file__), f'dumped/{self.name}.pkl'), 'wb'))
                print("保存成功")
            else:
                print("未收集数据，不保存")
...
class StatisticalRevisionForEach(StatisticalFilter):
    ...
    def __remove_baseline(self):
        dataset_x = np.array(self.dataset_x)
        base_q = 0.01
        # for q in np.arange(0., 0.2, 0.02):
        #     quantile = np.quantile(dataset_x, q + base_q, axis=0) - np.quantile(dataset_x, base_q, axis=0)
        #     plt.imshow(quantile)
        #     plt.title(f'Distribution of {self.name}, Quantile: {q:.2f}')
        #     plt.xlabel('Sensor Index')
        #     plt.show()
        dataset_x = np.quantile(dataset_x, base_q, axis=0)
        return dataset_x
...
class OverallCalibrator(Filter):
    pass
