...
class LargeSensorDriverReduced(AbstractSensorDriver):
    # 传感器驱动

    SENSOR_SHAPE = (32, 32)  # 形状

    SCALE = 1.

    def __init__(self):
        super(LargeSensorDriverReduced, self).__init__()
    ...
    def get(self):
        ...
        if self.sensor_backend.buffer:
            ...
            data = bits_and_t[0][0].reshape(self.SENSOR_SHAPE).astype(np.int16)
            t = bits_and_t[1]
            return data, t
        else:
            return None, None
...
MESSAGE_SIZE = 1024
PACKAGE_SIZE = 40  # 256  #0xaa10/0x3388(4)+ frame_cnt(1) + packet_cnt(1)+ 64 + crc16(2)
PACKAGE_COUNT_IN_FRAME = 32
SENSOR_SIZE = 32
BYTES_PER_POINT = 1
POINTS_PER_PACKAGE = 32

# FOLDING = config_array["column_array"][:SENSOR_SIZE]
    ...
    def read(self):
        ...
        while offset < (m - PACKAGE_SIZE):  # range(0,MESSAGE_SIZE-):     #[OFFSET_0, OFFSET_1]:
            ...
            if (self.message_cache[offset] == 0xaa) & (self.message_cache[offset + PACKAGE_SIZE] == 0xaa):
                ...
                if self.last_frame_number is None:  # last_frame_number尚未被设置过
                    flag = (package_number == 0)  # 有效
                    if flag:
                    ...
                    if package_number == 0:
                        if self.last_package_number == PACKAGE_COUNT_IN_FRAME - 1:  # 上一帧完成了
                            self.__finish_frame()
                            flag = True
                        else:
                            self.warn_info = '包号错误'
                ...
                if flag:
                    ...
                    for bit, slice_from in enumerate(slices_from):
                        ...
                        self.preparing_frame[bit][slice_to] = self.message_cache[slice_from]
                        pass
                        # 记得验一下
                # else:
                # break
    ...
    def get_usb_devices(self, rev):
        ...
        for dev in devs:
            if dev.bcdDevice == rev:
                return dev
        raise Exception('未找到USB设备，或设备号设置不正确')
